...
export async function submitAnswer(req, res) {
  try {
    ...
    if (
      ...
      !isValidObjectId(locationId) ||
      !isValidObjectId(huntId)
    ) {
    ...
    if (existingAnswer) {
      return res.status(400).json({
        status: "failed",
        message:
          "You have already submitted an answer for this location in this hunt!",
      });
    }
...
export async function updateAnswerById(req, res) {
  ...
  const actualUpdated = Object.keys(updates).filter((key) =>
  ...
  try {
    ...
    answer.evaluationScore = await evaluateResponse(
      answer.answer,
      correctAnswers
    );
