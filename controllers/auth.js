  ...
  try {
    ...
    if (existingUser)
      return res.status(409).json({
        status: "failed",
        message:
          "An account with that email is already in the process of being created!",
      });
    //console.log(email);
...
export async function VerifyEmail(req, res) {
  ...
  try {
    ...
    const tempUser = await TemporaryRegistration.findOne({
      email,
      verificationCode,
    });
...
export async function Logout(req, res) {
  try {
    ...
    if (checkIfBlacklist) return res.sendStatus(204);
...
///Change password based on email and password
